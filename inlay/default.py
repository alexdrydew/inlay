...
def default_rules() -> RuleGraph:
    ...
    strict_variant_ref = builder.lazy(lambda: strict_variant_pipeline)
    ...
    def pipeline_for(
        *,
        auto_target: Rule,
        union_variant_rules: Rule,
        include_sentinel_none: bool = True,
    ) -> Rule:
        ...
        sentinel_rules = (
            (sentinel, *registry_rules) if include_sentinel_none else registry_rules
        )
    ...
    pipeline = pipeline_for(auto_target=self_ref, union_variant_rules=self_ref)
    strict_pipeline = pipeline_for(
        auto_target=strict_ref,
        union_variant_rules=strict_variant_ref,
    )
    strict_variant_pipeline = pipeline_for(
        auto_target=strict_variant_ref,
        union_variant_rules=strict_variant_ref,
        include_sentinel_none=False,
    )
