    ...
    solver_in_span!(
        ...
        },
        {
            ...
            let (lookups, dependencies, cross_env_reuses, result_ref) = {
                let mut rule_ctx = crate::rule::RuleContext::new(
                    rule,
                    goal.state_id,
                    goal.env,
                    ctx,
    ...
    solver_in_span!(
        ...
        },
        {
            ...
            if ctx.cache_reuse_enabled {
                ...
                if let Some((exact_result_refs, bucket_len, entries)) =
                    ctx.cache.get(&key).map(|bucket| {
                        (
                    ...
                    })
                {
                    ...
                    for entry in entries.iter().rev() {
                        ...
                        let matched = answer_matches_env(
                            rule,
                            entry.result_ref,
                            &goal.env,
                            ctx,
...
pub fn solve<R: Rule>(
    ...
    let result = solver_in_span!(
        ...
        {
            solve_goal(rule, root_goal, &mut ctx)
